...
/*
 * Each block has a 64-bit checksum -- using CRC32 for better performance.
 */
typedef struct zio_cksum {
	uint64_t	zc_word[1];	/* 64-bit checksum */
} zio_cksum_t;
...
typedef enum zio_checksum {
	ZIO_CHECKSUM_OFF = 0,
	ZIO_CHECKSUM_ON,
	ZIO_CHECKSUM_CRC32,	/* CRC32 using SSE4.2 acceleration */
	ZIO_CHECKSUM_LAST
} zio_checksum_t;
...
/* SSE4.2 CRC32 acceleration */
#ifdef __x86_64__
#include <nmmintrin.h>  /* SSE4.2 intrinsics */

static inline uint32_t
crc32_sse42(const void *buf, size_t len, uint32_t crc)
{
	const uint8_t *p = buf;
	size_t i;
...
/* Table-based CRC32 software implementation */
static inline uint32_t
crc32_table_sw(const void *buf, size_t len, uint32_t crc)
{
	const uint8_t *p = buf;
	size_t i;
...
/* Direct calculation CRC32 software implementation */
static inline uint32_t
crc32_direct_sw(const void *buf, size_t len, uint32_t crc)
{
	const uint8_t *p = buf;
	size_t i;
	uint32_t bit;
...
/* CRC32 checksum calculation with multiple implementation options */
static inline void
zio_cksum_crc32(zio_cksum_t *zcp, const void *buf, size_t size)
{
	...
	if (__builtin_cpu_supports("sse4.2")) {
		crc = crc32_sse42(buf, size, crc);
	} else {
		/* Use table-based implementation as fallback */
		crc = crc32_table_sw(buf, size, crc);
	}
#else
	/* Use table-based implementation on non-x86 platforms */
	crc = crc32_table_sw(buf, size, crc);
#endif

	/* Final XOR */
	crc ^= 0xFFFFFFFF;
	zcp->zc_word[0] = (uint64_t)crc;
}
